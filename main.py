...
warnings.filterwarnings("ignore",
                        message="No frequency information was provided, so inferred frequency B will be used.")

# Constants
FORECAST_DAYS_MAX = 180
...
def compute_technical_score_original(close_px, sma50, sma200, rsi_val, macd_val, macd_sig, bb_up, bb_lo, atr_val,
                                     stoch_k, open_px):
    score = 0.0
...
def compute_technical_score_enhanced(close_px, sma50, sma200, rsi_val, macd_val, macd_sig, bb_up, bb_lo, atr_val,
    ...
    if close_px is not None and sma200 is not None and close_px > sma200:
        trend_score += 15
    if (ichimoku_span_a is not None and ichimoku_span_b is not None and close_px is not None and
...
def forecast_volume_statsmodels(data: pd.DataFrame, forecast_periods: int) -> pd.DataFrame:
    volume_series = data['Volume'].astype(float).replace(0, np.nan).ffill()
    volume_log = np.log(volume_series)
    model = SARIMAX(volume_log, order=(1, 1, 1), enforce_stationarity=False, enforce_invertibility=False)
    results = model.fit(disp=False)
    forecast_log = results.forecast(steps=forecast_periods)
    forecast_volumes = np.exp(forecast_log)
    last_date = data.index[-1]
    date_range = pd.date_range(start=last_date, periods=forecast_periods + 1, freq='B')[1:]
    return pd.DataFrame({'Forecast_Volume': forecast_volumes.values}, index=date_range)


def forecast_price_prophet(data: pd.DataFrame, forecast_periods: int) -> pd.DataFrame:
    naive_index = data.index.tz_localize(None)
    prophet_df = pd.DataFrame({'ds': naive_index, 'y': np.log(data['Close'].values)})
    m = Prophet(yearly_seasonality=True, weekly_seasonality=True, daily_seasonality=False)
    m.fit(prophet_df)
    future_dates = pd.date_range(start=naive_index[-1], periods=forecast_periods + 1, freq='B')[1:]
    future_df = pd.DataFrame({'ds': future_dates})
    forecast = m.predict(future_df)
    predicted_price = np.exp(forecast['yhat'].values)
    return pd.DataFrame({'Prophet_Price': predicted_price}, index=future_dates)
...
def print_detailed_fundamentals(ticker: str, fundamentals: dict) -> dict:
    ...
    for key, value in keys:
        ...
        if value is not None:
            if key in ["Market Cap", "Enterprise Value", "Revenue (ttm)",
                       "Net Income Avl to Common (ttm)", "Total Cash (mrq)",
...
def generate_chart_html(df: pd.DataFrame, ticker: str, chart_type: str) -> str:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['RSI'],
                  ...
                  row=3, col=1)
    fig.add_hrect(y0=30, y1=70, fillcolor="gray", opacity=0.2, line_width=0,
                  row=3, col=1)
...
# -------------------------------------------------------------------
# Updated run_all_web Function with Separate Analytics and Drawing Variables
# -------------------------------------------------------------------
def run_all_web(ticker, start_date, end_date, chart_type, forecast_days_max, horizons):
    buf = io.StringIO()
...
def analyze_data(df: pd.DataFrame, ticker: str, chart_type: str, start_date, end_date: datetime):
    ...
    volume_forecast = forecast_volume_statsmodels(df, FORECAST_DAYS_MAX)
    price_forecast = forecast_price_prophet(df, FORECAST_DAYS_MAX)
    ...
    for horizon in ml_horizons:
        ...
        if len(train_df) > 50:  # Lower threshold to 50 if needed
            ...
            print("Insufficient data for ML-based forecast for this horizon.")

    vol_table = {h: safe_float(volume_forecast.iloc[h - 1]['Forecast_Volume'], None)
                 for h in HORIZONS if h <= len(volume_forecast)}
    px_table = {h: safe_float(price_forecast.iloc[h - 1]['Prophet_Price'], None)
                for h in HORIZONS if h <= len(price_forecast)}
    print("\n--- Volume Forecast (30/60/90/180) ---")
    print(pd.DataFrame([vol_table], index=['Volume Forecast']).to_string(
        formatters={str(k): '{:.2f}'.format for k in vol_table.keys()}))
    print("\n--- Price Forecast (30/60/90/180) ---")
    print(pd.DataFrame([px_table], index=['Price Forecast']).to_string(
        formatters={str(k): '{:.2f}'.format for k in px_table.keys()}))
    ...
    tech_score_enhanced = compute_technical_score_enhanced(
        close_px, sma50_val, sma200_val, rsi_val, macd_val, macd_sig, bb_up, bb_lo, atr_val, k_val, latest['Open'],
        vwap_val, span_a_val, span_b_val, adx_val, latest_vix
    )
